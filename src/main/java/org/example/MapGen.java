...
public class MapGen {
    ...
    public static final float CAMERA_ZOOM = 2.5f;
    ...
    public static class Room { public int x,y,w,h; public Room(int x,int y,int w,int h){this.x=x;this.y=y;this.w=w;this.h=h;} }
    ...
    private void generate() {
        for (int x = 0; x < MAP_WIDTH; x++) for (int y = 0; y < MAP_HEIGHT; y++) { floor[x][y] = false; roomId[x][y] = -1; }
        for (int i = 0; i < MAX_ROOMS; i++) {
            ...
            for (int xx = x; xx < x+w; xx++) for (int yy = y; yy < y+h; yy++) { floor[xx][yy]=true; roomId[xx][yy]=i; }
    ...
    private void connectRooms(Room a, Room b) {
        ...
        if(rng.nextBoolean()) { for(int x=Math.min(ax,bx); x<=Math.max(ax,bx); x++) floor[x][ay]=true; for(int y=Math.min(ay,by); y<=Math.max(ay,by); y++) floor[bx][y]=true; }
        else { for(int y=Math.min(ay,by); y<=Math.max(ay,by); y++) floor[ax][y]=true; for(int x=Math.min(ax,bx); x<=Math.max(ax,bx); x++) floor[x][by]=true; }
    }
    ...
    public void drawMap() {
        for(int x=0;x<MAP_WIDTH;x++) for(int y=0;y<MAP_HEIGHT;y++){
            if(floor[x][y]){
                ...
                float px=x*TILE_SIZE, py=y*TILE_SIZE;
                glBegin(GL_QUADS);
                glVertex2f(px,py);
